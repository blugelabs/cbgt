...
func TestGetCallersName(t *testing.T) {
	cn := getCallersName(0)
	if lastComponent(cn.filename) != "log_test.go" {
		t.Errorf("Expected fn=log_test.go, got %q",
			lastComponent(cn.filename))
	}
	if lastComponent(cn.funcname) != "log.TestGetCallersName" {
		t.Errorf("Expected func=log.TestGetCallersName, got %q",
...
func TestOutput(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fgYellow + "DEBU: " + "testing123" + reset, -1, false,
		},
		{
			func() {
		...
		},
		{
			...
			fgYellow + "TRAC: " + "testing123" + reset, -1, false,
		},
		{
			func() {
